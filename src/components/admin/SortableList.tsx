...
interface SortableLists<T> {
    ...
    orderChangeHandler: (newOrder: string[]) => Promise<{ error: boolean, message: string }>
}

export function SortableList<T extends { id: string }>({ lists, children, orderChangeHandler }: SortableLists<T>) {
            ...
            onDragEnd={(event) => {
                ...
                if (isSortable(source)) {
                    ...
                    startTransition(async () => {
                        const { error, message } = await orderChangeHandler(newItems.map((item) => item.id))
...
export function SortableItem({ children, id, index, isPending, listType }: { children: ReactNode, id: string, index: number, isPending: boolean, listType: "section" | "lesson" }) {
    ...
    return (
        <div
            className={cn("", isDragSource && "bg-white/5 border border-white/50", isPending && "opacity-50 cursor-not-allowed")}
            ref={ref}
